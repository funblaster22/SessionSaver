...
import {TabGroup} from "./global";
import {mapAndFilter} from "./lib.js";
...
chrome.runtime.onConnect.addListener(port => {
  ...
  port.onMessage.addListener(async cmd => {
    ...
    if (cmd[0] === "load") {
      const metadata = cmd[1] as TabGroup;
      // Is this too confusing?
      const tabIds = await Promise.all(
        mapAndFilter(await chrome.bookmarks.getChildren(metadata.id), node =>
          node.title !== 'metadata' && chrome.tabs.create({url: node.url}).then(tab => tab.id)
        )
      );
      const groupId = await chrome.tabs.group({tabIds: tabIds});
      chrome.tabGroups.update(groupId, {title: metadata.shortname, color: metadata.color});
      chrome.tabs.highlight({tabs: tabIds});  // Documentation says to use index, but seems to work fine with ID
    }
